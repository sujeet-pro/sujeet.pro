...
![Where each rendering strategy produces HTML — at build (SSG), at the edge (ISR, PPR shell), at the origin server (SSR), or in the browser (CSR).](./diagrams/rendering-spectrum-light.svg "Where each rendering strategy produces HTML — at build (SSG), at the edge (ISR, PPR shell), at the origin server (SSR), or in the browser (CSR).")
![Where each rendering strategy produces HTML — at build (SSG), at the edge (ISR, PPR shell), at the origin server (SSR), or in the browser (CSR).](./diagrams/rendering-spectrum-dark.svg)
